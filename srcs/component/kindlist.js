...
class DefaultTabBar extends Component {
    ...
    this.state = {
      ...
      rightData: [
        ...
        [
          ...
          {'name': '芋头', 'img': require('../img/kinds/cereals/YuTou.jpg')},
        ]
      ]
    }
  }
    ...
    for (let i = 0; i < this.state.leftCategoryData.length; i++) {
      leftDom.push(
        <TouchableOpacity activeOpacity={1} onPress={this.categoryClick.bind(this,i)} key={i}>
          <View key={i} style={{
            height: 100/ designHeight * ScreenHeight,
            backgroundColor: this.state.selectCategoryIndex == i ? 'white':'gray',
            justifyContent: 'center',
            alignItems: 'center',
            borderBottomWidth: this.state.selectCategoryIndex == i ? 0 : 1 / PixelRatio.get(),
            borderBottomColor: 'rgb(200,200,200)'
          }}>
            <Text>{this.state.leftCategoryData[i]}</Text>
          </View>
        </TouchableOpacity>
      )
    };
      ...
      rowItemDom.push(
        <View key={i} style={{alignItems: 'center',}}>
          <Image source={this.state.rightData[this.state.selectCategoryIndex][index][i].img} style={{
            height: 260 / designHeight * ScreenHeight,
            width: 160 / designWidth * ScreenWidth
          }} resizeMode={'cover'}/>
          <Text style={{
            marginTop: 10
          }}>
  ...
  // scrollowView 自动换行效果实现方式 一行显示三个商品，构造数据时
  render() {
    return (
